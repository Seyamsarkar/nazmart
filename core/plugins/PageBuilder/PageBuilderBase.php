...
use Plugins\PageBuilder\Helpers\Traits\LanguageFallbackForPageBuilder;
use Plugins\PageBuilder\Helpers\Traits\LoadScript;
...
{
    use LanguageFallbackForPageBuilder,RepeaterHelper,RenderViews, GlobalAdminFields,LoadScript;
    ...
    public function __construct(array $args=[])
    {
        ...
        $this->base_preview_image_path = global_asset('assets/plugins/PageBuilder/images');
        try {
    ...
    {
        return '<form method="post" action="' . route(route_prefix().'admin.page.builder.' . $this->args['type']). '" enctype="multipart/form-data"><input type="hidden" value="' . csrf_token() . '" name="_token">';
    }
    ...
    public function admin_render_raw(){
        $output = '';
    ...
    public function get_preview_image($image_name): string
    {
        ...
        return '<a href="'.$this->base_preview_image_path.'/'.$image_name.'" title="'.__('preview of this addon').'" class="preview-image" target="_blank"><i class="las la-image"></i></a>';
    }
}
