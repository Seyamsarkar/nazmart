...
class NavigationMenuWidget extends WidgetBase
{
    ...
    public function admin_render()
    {
        ...
        foreach ($all_languages as $key => $lang) {
            ...
            $widget_title =  SanitizeInput::esc_html($widget_saved_values['widget_title_'. $lang->slug]) ?? '';
        ...
        foreach($navigation_menus as $menu_item){
            ...
            $output .= '<option value="'.$menu_item->id.'" '.$selected.'>'.purify_html(SanitizeInput::esc_html($menu_item->title)).'</option>';
        }
        $output .= '</select>';
        $output .= '</div>';
