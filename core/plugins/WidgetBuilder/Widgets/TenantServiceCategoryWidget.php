...
class TenantServiceCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_categories = ServiceCategory::where('status',1)->orderBy('id', 'DESC')->take($category_items)->get();
        $category_markup = '';
        foreach ($blog_categories as $item){

            $title = SanitizeInput::esc_html($item->title);
            $url = route(route_prefix().'frontend.service.category', ['id' => $item->id,'any' => $title]);
