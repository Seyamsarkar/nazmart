...
<div class="mail-container">
    ...
    <div class="main-content-wrap">
        ...
        @if($extra == 'custom_sub')
           <h4 class="renew_heading"> <strong>{{__('You have been assinged a subscription please complete your payment (Please contact to the administrator)')}} </strong></h4>
        @endif
        ...
        <table>
            ...
            @if(!empty($package->start_date) && !empty($package->expire_date))
                ...
                <tr>
                    ...
                    <td>{{$package->expire_date?->format('d-m-Y H:m:s') ?? ''}}</td>
                </tr>
            @endif
            ...
            @if($user_type == 'user')
                ...
                <tr>
                    ...
                    <td>{{optional($package->user)->country }}</td>
                </tr>

                <tr>
                    <td><strong>{{__('Your Domain :')}}</strong></td>
                    <td>{{$package->tenant_id .'.'. current(config('tenancy.central_domains')) }}</td>
                </tr>

                <tr>
                    <td><strong>{{__('Your Site Url :')}}</strong></td>
                    <td>
                        <a href="{{ tenant_url_with_protocol($package->tenant_id .'.'. current(config('tenancy.central_domains')) ?? '' ) }}" target="_blank">
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ tenant_url_with_protocol($package->tenant_id .'.'. current(config('tenancy.central_domains')) .'/admin' ?? '' )  }}" target="_blank">
