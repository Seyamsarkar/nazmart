                    ...
                    <form action="{{route('tenant.shop.checkout')}}" method="POST" class="checkout-form" enctype="multipart/form-data">
                        ...
                        <div class="checkout-form-flex">
                            <div class="single-input mt-4">
                                ...
                                <input class="form--control" type="tel" placeholder="{{__('Type Mobile Number')}}" name="phone" value="@auth('web'){{$billing_info ? $billing_info->phone : auth('web')->user()?->mobile}}@else{{old('phone')}}@endauth" {{$readonly}}>
                            </div>
                            <div class="single-input mt-4">
                        ...
                        <div class="checkout-form-flex">
                            ...
                            <div class="single-input mt-4">
                                ...
                                <input class="form--control" type="text" placeholder="{{__('Type City/Town')}}" name="city" value="@auth('web'){{$billing_info ? $billing_info->city : ''}}@else{{old('city')}}@endauth" {{$readonly}}>
                            </div>
                        </div>
                        <div class="checkout-form-flex">
                            <div class="single-input mt-4">
                                <label class="label-title mb-3"> {{__('Address')}} </label>
                                <textarea class="form--control form--message" name="address" placeholder="{{__('Type Address')}}" {{$readonly}}>@auth('web'){{$billing_info ? $billing_info->address : ''}}@else{{old('address')}}@endauth</textarea>
                            </div>
                        </div>
