                    ...
                    <div class="support-ticket-summery-warp">
                        <div class="gig-chat-message-heading">
                            <div class="header-wrap d-flex justify-content-between">
                                <h4 class="header-title">{{__('Refund Message Details')}}</h4>
                                <a href="{{route('tenant.user.dashboard.refund.chat.list')}}"
                                   class="btn btn-primary rounded-0">{{__('All Chats')}}</a>
                            </div>
                            <div class="gig-order-info">
                                <ul>
                                    <li><strong>{{__('Ticket ID:')}}</strong> #{{$chat->id}}</li>
                                    <li><strong>{{__('Title:')}}</strong> {{$chat->title}}</li>
                                    <li><strong>{{__('Subject:')}}</strong> {{$chat->subject}} </li>
                                    <li><strong>{{__('Status:')}}</strong> <span class="status-{{$chat->status}}">{{$chat->status}}</span>
                                    </li>
                                    </li>
                                    <li>
                                        <strong>{{__('Description:')}}</strong> {{$chat->description}}
                                    </li>
                                </ul>
                            </div>
                                    ...
                                    @forelse($all_messages as $msg)
                                        <div class="single-message-item @if($msg->type == 'customer') customer @endif">
                                            ...
                                            <div class="content">
                                                ...
                                                @if(file_exists('assets/uploads/refund_chat/'.$msg->attachment))
                            ...
                            <div class="reply-message-wrap ">
                                    ...
                                    <form action="{{route('tenant.user.dashboard.chat.message.show', $chat->id)}}" method="post"
                                          enctype="multipart/form-data">
                                        @csrf
                                        <input type="hidden" value="{{$chat->id}}" name="ticket_id">
                            ...
                            @else
                                <div class="reply-message-wrap ">
                                    <h4 class="text-center">{{__('The Conversation is Closed')}}</h4>
                                </div>
                            @endif
                        </div>
                    </div>
